...
public class RefreshableView extends LinearLayout implements OnTouchListener {
	...
	/**预加载*/
	public static final int PRELOAD=6688;
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		if (changed && !loadOnce) {
			...
			int i=0;
			while(i<count){
				if(this.getChildAt(i) instanceof ListView){
	...
	/**调用此方法支持上拉加载更多*/
	private void supportPullToLoad(){
		
		footer = getChildAt(getChildCount()-1);
		footerLayoutParams=(MarginLayoutParams) footer.getLayoutParams();
		
		footerProgressBar = (ProgressBar) footer.findViewById(R.id.progress_bar);
		footerArrow = (ImageView) footer.findViewById(R.id.arrow);
		footerDescription = (TextView) footer.findViewById(R.id.description);
	}
...
//		}
		...
		if (ableToPull) {
			return pullHandle(event);
		}else if(ableToLoad){
			return loadHandle(event);
		}
		return false;
	}
...
//	}
	...
	private boolean loadHandle(MotionEvent event){
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (distance >= 0 && footerLayoutParams.topMargin >= 1280) {
				return false;
			}
			...
			if(currentStatus!=STATUS_EXECUTING){
				if(footerLayoutParams.bottomMargin<1280){
		...
		default:
		    if(ableToLoad){
				...
				} else if (currentStatus == STATUS_PULL_TO_LOAD) {
...
//			updateHeaderView();
	...
	private boolean pullHandle(MotionEvent event){
...
//			Log.e("majie", "pull ...");
	 ...
	 */
	public void setOnRefreshListener(PullToRefreshListener listener, int id,boolean isPreload) {
	...
	/**
	 * 根据当前ListView的滚动状态来设定 {@link #ableToPull}
	 * 的值，每次都需要在onTouch中第一个执行，这样可以判断出当前应该是滚动ListView，还是应该进行上拉。
	 * 
	 * @param event
	 */
		...
		if (listView.getChildAt(listView.getLastVisiblePosition() - listView.getFirstVisiblePosition()) != null  
                && listView.getChildAt(  
                		listView.getLastVisiblePosition()  
                                - listView.getFirstVisiblePosition()).getBottom() <= listView.getMeasuredHeight()) {
	...
	private void disableToPull(){
		ableToPull=false;
	}
	
	private void disableToLoad(){
		ableToLoad=false;
	}
	...
	private void updateFooterView() {
		if (lastStatus != currentStatus) {
			if (currentStatus == STATUS_PULL_TO_LOAD) {
				footerDescription.setText(getResources().getString(R.string.pull_to_refresh));
				footerArrow.setVisibility(View.VISIBLE);
				footerProgressBar.setVisibility(View.GONE);
				rotateArrow();
			} else if (currentStatus == STATUS_RELEASE_TO_LOAD) {
				footerDescription.setText(getResources().getString(R.string.release_to_refresh));
				footerArrow.setVisibility(View.VISIBLE);
				footerProgressBar.setVisibility(View.GONE);
				rotateArrow();
			} else if (currentStatus == STATUS_EXECUTING) {
				footerDescription.setText(getResources().getString(R.string.load));
	...
	class LoadingTask extends AsyncTask<Void, Integer, Void>{
		...
		protected Void doInBackground(Void... arg0) {
			...
			while (true) {
				...
				if (topMargin > 2*hideHeaderHeight) {
					topMargin = 2*hideHeaderHeight;
					break;
				}
				publishProgress(topMargin);
				sleep(10);
			}
			currentStatus = STATUS_EXECUTING;
			publishProgress(2*hideHeaderHeight);
